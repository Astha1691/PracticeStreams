...
public class BankTransactionExample {
    ...
    private static Optional<BankTransaction> getSmallestTransaction(LocalDate when) {
        return transactions.stream().filter(transaction -> transaction.getDate().equals(when)).min(Comparator.comparing(BankTransaction::getAmount));
    }

    private static Optional<BankTransaction> getBiggestTransaction(LocalDate when) {
       return transactions.stream().filter(transaction -> transaction.getDate().equals(when)).max(Comparator.comparing(BankTransaction::getAmount));
    }

    private static long getTotalTransactionsToday() {

       return transactions.stream().filter(transaction->transaction.getDate().equals(LocalDate.now())).count();
    }
